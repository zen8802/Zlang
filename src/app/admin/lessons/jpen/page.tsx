      ...
      <div className="max-w-3xl mx-auto px-4 py-8">
        ...
        {Object.entries(units).sort(([a], [b]) => Number(a) - Number(b)).map(([unit, unitLessons]) => (
          <div key={unit} className="mb-6">
            <h2 className="text-xs font-semibold text-[#9CA3AF] uppercase tracking-widest mb-3 px-1">Unit {unit}</h2>
            <div className="flex flex-col gap-2">
              {unitLessons.map(lesson => (
                <Card key={lesson.id} variant="elevated" onClick={() => router.push(`/admin/editor/${lesson.id}`)} padding="md">
                  <div className="flex items-center gap-3">
                    <div className="w-9 h-9 rounded-[10px] bg-[#EBF0F8] text-[#1B4F8A] flex items-center justify-center font-semibold text-sm shrink-0">{lesson.order}</div>
