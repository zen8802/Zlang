...
export default function EnJpLessonsPage() {
  ...
  useEffect(() => {
    fetch('/api/admin/lessons')
      ...
      .then(data => {
        ...
        setLessons(all.filter(l => l.target_language === 'japanese'))
      })
      .catch(() => setLessons([]))
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        ...
                        {lesson.blocks?.length > 0 && (
                          <span className="text-xs text-[#9CA3AF]">
                            {lesson.blocks.length} blocks {lesson.blocks.slice(0, 5).map(b => BLOCK_ICONS[b.type as string] || '').join('')}
                          </span>
                        )}
                      </div>
                    </div>
